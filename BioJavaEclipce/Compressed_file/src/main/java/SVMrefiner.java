...
public class SVMrefiner implements MllLibRefiner {
	...
	public Transcriptome getGoodTranscripts(Assignment trAssignment1,
		...
		File file1 = new File(path + 
				tr1Name + 
				" against " + tr2Name + 
				" good reads classifier SVM tb" + topBound
				+ " bb" + bottomBound);
		...
		if (file1.exists() && file2.exists()){
			...
			classifier1 = (SMO) in1.readObject();
			...
			FileInputStream fis2 = new FileInputStream(path + 
					tr2Name + 
					" against " + tr1Name + 
					" good reads classifier RF tb" + topBound
					+ " bb" + bottomBound);
			
			ObjectInputStream in2 = new ObjectInputStream(fis2);
			classifier2 = (SMO) in2.readObject();
			System.out.println("Classifier 1 red");
			in2.close();
		}
		else {
		...
		for (Transcript indexOfTr : tr1.getAllTranscripts()){
			...
			System.out.println("Percentage: " + (count - goodCount)/count);
		...
		System.out.println("Transcripts, obtained "
				+ "from \"good\" reads for" 
				+ trAssignment1.getNameOfSet1());
		System.out.println("Minimal art of \"good\" "
				+ "reads in each transcript: " + percentage);
		...
		for (Transcript indexOfTr : tr2.getAllTranscripts()){
			...
			if ((count - goodCount)/count > percentage){
				System.out.println(indexOfTr.getData());
				System.out.println(indexOfTr.getName());
				results.addTranscript(indexOfTr);
				System.out.println("Transcript added!");
				blabla++;
			}
